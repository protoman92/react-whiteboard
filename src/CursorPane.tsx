...
export class CursorPane extends React.Component<Props, State> {
  ...
  onMouseMoveCursorLayer(ev: React.MouseEvent<HTMLDivElement>) {
    ...
    if (this.props.mode === ModeEnum.DRAW_LINE) {
        ...
        const moveX =
          lastImage.x + unit < 0
            ? ev.nativeEvent.offsetX -
              ...
              (lastImage.x + unit)
            : ev.nativeEvent.offsetX - this.state.dragStart.x;
        const moveY =
          lastImage.y + unit < 0
            ? ev.nativeEvent.offsetY -
              this.state.dragStart.y -
              (lastImage.y + unit)
            : ev.nativeEvent.offsetY - this.state.dragStart.y;

        this.props.events.dragImage(moveX, moveY);
      }
    } else if (
      this.props.mode === ModeEnum.NW_RESIZE_IMAGE ||
      this.props.mode === ModeEnum.NE_RESIZE_IMAGE ||
      this.props.mode === ModeEnum.SE_RESIZE_IMAGE ||
      this.props.mode === ModeEnum.SW_RESIZE_IMAGE
    ) {
      const lastImage = this.props.eventStore.lastImage();
      if (!lastImage) {
        return;
      ...
      if (this.state.resizeStart) {
        ...
        if (
          this.props.mode === ModeEnum.NW_RESIZE_IMAGE &&
          (lastImage.width - moveX < 0 || lastImage.height - moveY < 0)
        ) {
          return;
        } else if (
          this.props.mode === ModeEnum.NE_RESIZE_IMAGE &&
  ...
  onClickDragHandle(ev: React.MouseEvent<HTMLDivElement>) {
    if (this.props.mode === ModeEnum.HAND) {
      this.setState({
        dragStart: { x: ev.nativeEvent.offsetX, y: ev.nativeEvent.offsetY },
      });
  ...
  onClickResizeHandle(
    direction: ResizeImageDirection,
    ev: React.MouseEvent<HTMLDivElement>
  ) {
    if (this.props.mode === ModeEnum.HAND) {
      this.setState({ resizeStart: { x: ev.pageX, y: ev.pageY } });
  ...
  renderImageHandle(): Array<React.ComponentElement<any, any>> | undefined {
    ...
    const base =
      lastImage.width < lastImage.height ? lastImage.width : lastImage.height;
    const unit = base / 8 < 20 ? Math.ceil(base / 8) : 20;
    ...
    const bottom = mathMinOrMax(
      0,
      this.props.height,
      lastImage.y + lastImage.height - unit
    );
    const left = mathMinOrMax(0, this.props.width, lastImage.x + unit);
    const right = mathMinOrMax(
      0,
      this.props.width,
      lastImage.x + lastImage.width - unit
    );
