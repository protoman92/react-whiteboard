...
export class Whiteboard extends React.Component<Props, State> {
  ...
  setupEventHandler() {
    this.props.events.on(
      EventNameEnum.SELECT_LAYER,
      this.selectLayer.bind(this)
    );
    ...
    this.props.events.on(
      EventNameEnum.SET,
      (event: ChangeStrokeWidth | ChangeStrokeColor) => {
  ...
  startDrawing(point: PointData) {
    this.state.eventStore.startDrawing(
  ...
  pushPoint(point: PointData) {
    this.state.eventStore.pushPoint(
      this.state.strokeWidth,
      this.state.strokeColor,
      point
    );
