...
export const isReducedLineEvent = (
  arg: AnyReducedEvent
): arg is ReducedLineEvent => {
  return arg.type === SvgElementEnum.LINE;
};

export const isReducedImageEvent = (
  arg: AnyReducedEvent
): arg is ReducedImageEvent => {
  return arg.type === SvgElementEnum.IMAGE;
};
...
export class EventStore implements EventStoreProtocol {
  ...
  reduceEvents(): Array<AnyReducedEvent> {
    return this.goodEvents
      .reduce(
        (
          prev: Array<Array<AnyReducedEvent>>,
          element: AnyEvent
        ): Array<Array<AnyReducedEvent>> => {
          if (!element.type) {
            prev.forEach((p) => {
              p.push({});
            });
      ...
      .reduce(
        (
          prev: Array<AnyReducedEvent>,
          element: Array<AnyReducedEvent>
        ): Array<AnyReducedEvent> => {
